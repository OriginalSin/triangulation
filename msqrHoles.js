...
function MSQR(src, options) {
	...
	if (max > 1 || padding) {
		...
		do {
			...
			if (path.length) {
				...
				// add to list
				var path2d = points2path(path);
				paths.push(retPath ? points2path(path) : path);
				if (!options.holes) {
					var bbox = MSQR.getBounds(path);
					holes.push(traceHoles(path2d, bbox));
				}
	...
	function traceHoles(path2d, bbox) {
		...
		ctx1.lineWidth = bleed;
		ctx1.miterLimit = 1;

		ctx1.putImageData(ctx.getImageData(cx, cy, cw, ch), 0, 0);
		// ctx1.putImageData(ctx.getImageData(bbox.x, bbox.y, bbox.width, bbox.height), 0, 0);
		...
		for (var y = bbox.y; y < my; ++y) {
			for (var x = bbox.x; x < mx; ++x) {
				var nm = y * cw + x,
					p = data[nm],
					value = skipVal;
				...
				if (p === 0 &&
					(p>>>24 === 0) &&
					ctx1.isPointInPath(path2d, x, y)
					&& !ctx1.isPointInStroke(path2d, x, y)
				) {
					// if (p>>>24 === 0) {
						value = val;
					// }
